...
a <- function(x)
{
  return (subset(x, select=c(Score, Close, NextReturns, PE, PB, Returns, ClosePercentile, PEPercentile, VolumnPercentile, MomentumPercentile, AmplitudePercentile, Breakdown, Breakup)))
}

load.data.df <- function(file) { data <- read.table(file, header= TRUE, sep=",", stringsAsFactors = FALSE, 
  colClasses=c("factor", "NULL", "character", "numeric", "numeric", "numeric", "numeric", "numeric", "numeric", "numeric", "numeric", "numeric", "numeric", "NULL", "NULL", "NULL", "NULL", "NULL", "numeric" ), 
  ...
  data <- subset(data, select=c(Date, Open, Close, High, Low, Volumn, PE, PB, IR)) 
  ...
  reversed.Momentum = rev(returns * data$Volumn) 
  y <- year(data$Date) 
  m <- month(data$Date) 
  wd <- wday(data$Date) - 1  
  md <- mday(data$Date)
  ...
  momentum.percentile <- one.year.percenctile(reversed.Momentum)
  two.week.breakdown <- two.week.breakdown(reversed.Close)
  two.week.breakup <- two.week.breakup(reversed.Close)
  data <- transform(data, 
                    Returns = returns,
                    Year = y,
                    Month = m,
                    ...
                    MA20 = rev(ma20),
                    MA30 = rev(ma30),
                    PrevClose = lead(data$Close),
                    NextReturns = (lag(Close, 5) - Close) * 100 / Close,
                    ClosePercentile = rev(close.percentile),
                    PEPercentile = rev(pe.percentile),
                    VolumnPercentile = rev(volumn.percentile),
                    MomentumPercentile = rev(momentum.percentile),
                    AmplitudePercentile = rev(amplitude.percentile),
                    Breakdown = rev(two.week.breakdown),
                    Breakup = rev(two.week.breakup))
  score <- (1 - sqrt(data$PEPercentile)) * 5
  score <- ifelse(data$MA10 > data$MA20, score + 3, score - 3)
  score <- ifelse(data$Close > data$MA10, score + 4, score - 4)
  score <- ifelse(data$Breakdown == TRUE, score - 2, score)
  score <- ifelse(data$Breakup == TRUE, score + 1, score)
  data <- transform(data, 
                    PrevMA10 = lead(MA10),
                    PrevMA20 = lead(MA20),
                    PrevMA30 = lead(MA30),
...
returns <- round(diff(stocks$Close) / stocks$Close[1:length(stocks$Close)], 6)
 ...
 positions <- quantile(stocks$PE, c(1 - 0.8, 1-0.4, 1-0.2), names = FALSE)
 abline(h = positions)
 axis(4, at=positions,labels=c(20, 60, 80), las=2, cex.axis=0.7)
...
down.ma10 <- stocks.whole[stocks.whole$Close < stocks.whole$MA10 & stocks.whole$PrevClose >= stocks.whole$PrevMA10]
up.ma10 <- stocks.whole[stocks.whole$Close >= stocks.whole$MA10 & stocks.whole$PrevClose < stocks.whole$PrevMA10]
...
down.ma20 <- stocks.whole[stocks.whole$Close < stocks.whole$MA20 & stocks.whole$PrevClose >= stocks.whole$PrevMA20]
up.ma20 <- stocks.whole[stocks.whole$Close >= stocks.whole$MA20 & stocks.whole$PrevClose < stocks.whole$PrevMA20]
...
down.ma30 <- stocks.whole[stocks.whole$Close < stocks.whole$MA30 & stocks.whole$PrevClose >= stocks.whole$PrevMA30]
up.ma30 <- stocks.whole[stocks.whole$Close >= stocks.whole$MA30 & stocks.whole$PrevClose < stocks.whole$PrevMA30]
